...
filestr = "nbt_convert_errorlog" + "_" + timestr + ".txt"
errorlog = os.path.join(cwd,filestr)
...
#                                 "fieldmaps are part of the dataset, choose either [all] or [single].")
                    ...
                    if len(outfile)>0 and not args.overwrite:
                        print("Target file " + outfile[0] + " already exists. File will not be overwritten.")
                        continue
                    elif len(outfile)>0 and args.overwrite:
                        print("Target file " + outfile[0] + " already exists. File will be overwritten.")
                        sysstr = 'rm -r -f ' + outfile[0]
                        os.system(sysstr)
                    ...
                    ############################################################## 
                    # dcm2niix: Data is copied to temp directory in home directory
                    ##############################################################

                    filename = os.path.basename(scan_ori)
                    homepath  = os.environ.get("HOME")
                    temp_path = os.path.join(homepath, 'temp')
                    temp_ori = os.path.join(homepath, 'temp', filename)
                    temp_dest = os.path.join(homepath, 'temp', 'dest')


                    sysstr = 'mkdir -p ' + temp_dest
                    os.system(sysstr)
                    sysstr = 'cp -r ' + ' ' + scan_ori + ' ' + temp_ori
                    os.system(sysstr)
                
                    dcm2niix_args = '-o ' + temp_dest + ' -b y -z y -f '
                    sysstr = dcm2niix_cmd + ' ' + dcm2niix_args + subsesprefix + scan_descr + ' ' + temp_ori
                    print(sysstr)
                    os.system(sysstr)
                  ...
                  #  os.system(sysstr)
                    ...
                    sysstr = 'cp ' + temp_dest + '/* ' + path_dest_nii
                    os.system(sysstr)
                    sysstr = 'rm -rf ' + temp_path
                    os.system(sysstr)
                    ...
                    if (mod == 'bold' or mod == 'sbref') and 'echos' in cur_scan:
                      ...
                      ########################################################################################################
                      #It seems dcm2niix returns an additional 4d nii image (+json file) with the wrong number of slices     #
                      #when processing ME data. Don't know why but these files are removed.                                  #
                      ########################################################################################################

                            echo_file_old_error_nii = path_dest_nii + '/' + subsesprefix + scan_descr + '_e' + str(i + 1) + 'a.nii.gz'
                            echo_file_old_error_json = path_dest_nii + '/' + subsesprefix + scan_descr + '_e' + str(i + 1) + 'a.json'
                            ...
                            if os.path.exists(echo_file_old_error_json):
                                sysstr = 'rm ' + echo_file_old_error_json
                                print(sysstr)
                                os.system(sysstr)
